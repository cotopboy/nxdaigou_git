...
namespace daigou.infrastructure.Extenders
{
    /// <summary>
    ...
    public class ListItemComparer : Comparer<object>
    {
        ...
        {
            var propChain = propName.Split('.').ToList();
          

            Type tempType = targetType;
            PropertyInfo propInfo = null;
            string tempPropName = propName;
            foreach (var item in propChain)
            {

                propName = item;
                propInfo = tempType.GetProperty(item, BindingFlags.Instance | BindingFlags.Public);

                if(item != propChain.Last())
            ...
            var convertedExp = System.Linq.Expressions.Expression.Convert(paExp, tempType);
